...
| `packages/api/src/services/threshold-gate.ts:36-37,92-97` | Extend `transition_to_closed_won` and `transition_to_closed_lost` action handlers to insert a `Deal` row (status=`closed_won`/`closed_lost`, `closedAt=now()`, value/currency from `Contact.metadata?.dealValue`/`dealCurrency` per Edit 4 / Q9.3, else `null`/`USD`) **alongside** the existing Pipeline stage transition. **Idempotent via `correlationId = decision.id`** per Edit 2's idempotency contract — if a `Deal` already exists with this `correlationId`, the write is a no-op (returns existing). Decision id changes per decision firing, so the same contact can have multiple closed_won deals across deal cycles (one per decision), but the same decision firing twice creates exactly one deal. Supersedes the prior `(tenantId, contactId, status='closed_won')` upsert language — that approach was inconsistent with Edit 2 and would have prevented multi-cycle closed-won, which is wrong. |
